 ...
 * Unknown characters are skipped (mapping them to ID 0 would inject the
 * pad/BOS/EOS token mid-sequence and corrupt synthesis).
 * encode() wraps the sequence: [0, ...ids..., 0]
 */
object KokoroPhonemeTokenizer {

    private const val SPECIAL_ID = 0  // "$" — pad / unk / sequence wrapper
    private const val TAG = "KokoroPhonemeTokenizer"
    ...
    fun encode(phonemes: String): IntArray {
        ...
        for (ch in phonemes) {
            ...
            if (id != null) {
                ids.add(id)
            } else if (warnedChars.add(ch)) {
                Log.w(TAG, "Dropping character not in Kokoro vocab: U+%04X".format(ch.code))
            }
        }
        ids.add(SPECIAL_ID)
    ...
    private val warnedChars = HashSet<Char>()
}
